...
local DnsLookupSDK = {}
DnsLookupSDK.__index = DnsLookupSDK
...
DnsLookupSDK._make_feature = _make_feature


function DnsLookupSDK.new(options)
  local self = setmetatable({}, DnsLookupSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DnsLookupSDK:options_map()
...
function DnsLookupSDK:get_utility()
  return Utility.copy(self._utility)
end


function DnsLookupSDK:get_root_ctx()
  return self._rootctx
end


function DnsLookupSDK:prepare(fetchargs)
...
function DnsLookupSDK:direct(fetchargs)
...
-- by passing it.
function DnsLookupSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DnsLookupSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DnsLookupSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DnsLookupSDK:DnsResult(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Domain(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:EmailValidate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Generate(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Grammar(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Ipn(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Redact(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Ssl(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Utility(data)
...
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
function DnsLookupSDK:Whoi(data)
...
function DnsLookupSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DnsLookupSDK.new(sdkopts)
  sdk.mode = "test"
