// Command DnsLookup-cli is a CLI + REPL for the DnsLookup SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DNS_LOOKUP_APIKEY (api key), DNS_LOOKUP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DNS_LOOKUP_APIKEY carries the API key and
	// DNS_LOOKUP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DNS_LOOKUP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DNS_LOOKUP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDnsLookupSDK(opts)
