...
boru-driven command-line client **and** interactive REPL for the DnsLookup
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dns-lookup-cli)
make build
...
./dns-lookup-cli load 1 dns_result            # {id:1} shorthand
./dns-lookup-cli load '{id:1}' dns_result       # explicit match map

# 5. Override the API base URL for a single call
DNS_LOOKUP_BASE=https://api.example.com ./dns-lookup-cli load 1 dns_result

# 6. No arguments -> interactive REPL
./dns-lookup-cli
dns-lookup> load 1 dns_result
dns-lookup> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dns-lookup-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dns-lookup-cli load 1 dns_result
   ```
...
```sh
./dns-lookup-cli load 1 dns_result          # scalar shorthand for {id:1}
./dns-lookup-cli load '{id:1}' dns_result     # explicit match map
```
...
./dns-lookup-cli load 1 dns_result
```
...
Run with no arguments to open a REPL (prompt `dns-lookup>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dns-lookup-cli`. |
...
(`./dns-lookup-cli <expr>`) and interactively (the REPL), and expressions compose the
