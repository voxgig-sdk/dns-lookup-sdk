...
{
    private static function utility(): DnsLookupUtility
    {
        return new DnsLookupUtility();
    }
    ...
    private static function ctx(array $over = []): DnsLookupContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DnsLookupContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DnsLookupOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DnsLookupError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DnsLookupOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DnsLookupMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DnsLookupOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DnsLookupMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DnsLookupOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DnsLookupMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DnsLookupSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DnsLookupMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DnsLookupResponse([]),
            'result' => new DnsLookupResult([]),
        ]);
        [, $err] = DnsLookupMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DnsLookupSpec([]),
            'response' => null,
            'result' => new DnsLookupResult([]),
        ]);
        [, $err] = DnsLookupMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DnsLookupSpec(['step' => 's']),
            'response' => new DnsLookupResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DnsLookupResult([]),
        ]);
        [, $err] = DnsLookupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DnsLookupSpec(['step' => 's']),
            'response' => new DnsLookupResponse(self::resp(200, ['v' => 1])),
            'result' => new DnsLookupResult([]),
        ]);
        [, $err] = DnsLookupMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DnsLookupSpec(['step' => 's']),
            'response' => new DnsLookupResponse(self::resp(200, ['v' => 2])),
            'result' => new DnsLookupResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DnsLookupMakeResponse::call($ctx);
    ...
    {
        $preset = new DnsLookupResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DnsLookupSpec([]),
            'response' => new DnsLookupResponse([]),
            'result' => new DnsLookupResult([]),
        ...
        [$got, $err] = DnsLookupMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DnsLookupResult([])]);
        [, $err] = DnsLookupMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DnsLookupSpec([]), 'result' => null]);
        [, $err] = DnsLookupMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DnsLookupSpec(['step' => 's']),
            'result' => new DnsLookupResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DnsLookupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DnsLookupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DnsLookupSpec(['step' => 's']),
            'result' => new DnsLookupResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DnsLookupOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DnsLookupMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DnsLookupResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DnsLookupSpec([]),
            'result' => new DnsLookupResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DnsLookupMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DnsLookupMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DnsLookupError('boom', 'boom');
        $utility->fetcher = function (DnsLookupContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DnsLookupSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DnsLookupMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DnsLookupContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DnsLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DnsLookupResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DnsLookupContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DnsLookupSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DnsLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DnsLookupResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DnsLookupSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DnsLookupMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DnsLookupMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DnsLookupMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DnsLookupResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DnsLookupDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DnsLookupResult(['ok' => false])]);
        $this->expectException(DnsLookupError::class);
        DnsLookupDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DnsLookupResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DnsLookupMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DnsLookupMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DnsLookupBaseFeature();
        $b = new DnsLookupBaseFeature();
        DnsLookupFeatureAdd::call($ctx, $a);
        DnsLookupFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DnsLookupBaseFeature
    {
        $f = new DnsLookupBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DnsLookupFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DnsLookupContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DnsLookupSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DnsLookupPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DnsLookupResponse(['headers' => null]),
            'result' => new DnsLookupResult([]),
        ]);
        DnsLookupResultHeaders::call($ctx);
