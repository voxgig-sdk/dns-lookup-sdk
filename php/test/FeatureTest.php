...
class FtCtrl extends DnsLookupControl
{
...
class FtClient
{
    ...
    private DnsLookupUtility $utility;

    public function __construct(DnsLookupUtility $utility, array $options)
    {
    ...
    public function get_utility(): DnsLookupUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DnsLookupContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DnsLookupContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DnsLookupContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DnsLookupConfig::make_feature($name);
    ...
    private static function buildUrl(DnsLookupSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DnsLookupError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DnsLookupResponse($fetched);
        }
    ...
    private function fail(DnsLookupContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DnsLookupContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
