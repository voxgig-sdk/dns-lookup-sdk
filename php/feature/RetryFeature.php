...
class DnsLookupRetryFeature extends DnsLookupBaseFeature
{
    ...
    public function init(DnsLookupContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (DnsLookupContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(DnsLookupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(DnsLookupContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
