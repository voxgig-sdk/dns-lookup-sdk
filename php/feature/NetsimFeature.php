...
class DnsLookupNetsimFeature extends DnsLookupBaseFeature
{
    ...
    private function _simulate(DnsLookupContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(DnsLookupContext $ctx, array $applied): void
    {
        $client = $this->client;
