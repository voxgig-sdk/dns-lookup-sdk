...
// DnsLookup SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class DnsLookupMakeError
{
    public static function call(?DnsLookupContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DnsLookupContext([], null);
        }
        $op = $ctx->op ?? new DnsLookupOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DnsLookupResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DnsLookupError) ? $err->msg : (string)$err;
        $msg = "DnsLookupSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new DnsLookupError('', $msg, $ctx);
