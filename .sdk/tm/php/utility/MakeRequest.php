...
class DnsLookupMakeRequest
{
    public static function call(DnsLookupContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new DnsLookupResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new DnsLookupResponse($fetched);
        } else {
