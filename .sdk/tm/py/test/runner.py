...
class DnsLookupTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DnsLookupTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DnsLookupTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DnsLookupTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = DnsLookupTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DnsLookupTestRunner.getenv(key)
        ...
        explain = DnsLookupTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DnsLookupTestRunner._test_control is not None:
            return DnsLookupTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DnsLookupTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DnsLookupTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DnsLookupTestRunner.load_test_control()
...
def load_env_local():
    DnsLookupTestRunner.load_env_local()


def env_override(m):
    return DnsLookupTestRunner.env_override(m)


def entity_data(v):
    return DnsLookupTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DnsLookupTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DnsLookupTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DnsLookupTestRunner.load_test_control()


def live_client_options():
    return DnsLookupTestRunner.live_client_options()


def live_delay_ms():
    return DnsLookupTestRunner.live_delay_ms()
