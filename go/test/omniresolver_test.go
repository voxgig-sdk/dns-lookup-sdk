...
import (
	...
	omni "github.com/voxgig-sdk/dns-lookup-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DnsLookupError); is {
				out := map[string]any{
					"name":    "DnsLookupError",
...
func omniCtx(arg any, client *sdk.DnsLookupSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DnsLookupSDK); is {
			client = live
