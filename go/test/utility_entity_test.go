...
func TestUtilityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DNSLOOKUP_TEST_UTILITY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func utilityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DNSLOOKUP_TEST_UTILITY_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"DNSLOOKUP_TEST_UTILITY_ENTID": idmap,
		"DNSLOOKUP_TEST_LIVE":      "FALSE",
		"DNSLOOKUP_TEST_EXPLAIN":   "FALSE",
		"DNSLOOKUP_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["DNSLOOKUP_TEST_UTILITY_ENTID"])
