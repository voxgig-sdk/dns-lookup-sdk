...
func TestDnsResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DNSLOOKUP_TEST_DNS_RESULT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func dns_resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DNSLOOKUP_TEST_DNS_RESULT_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"DNSLOOKUP_TEST_DNS_RESULT_ENTID": idmap,
		"DNSLOOKUP_TEST_LIVE":      "FALSE",
		"DNSLOOKUP_TEST_EXPLAIN":   "FALSE",
		"DNSLOOKUP_APIKEY":         "NONE",
	})

	idmapResolved := core.ToMapAny(env["DNSLOOKUP_TEST_DNS_RESULT_ENTID"])
