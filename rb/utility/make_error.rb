...
module DnsLookupUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = DnsLookupContext.new({}, nil)
    end
    op = ctx.op || DnsLookupOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || DnsLookupResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(DnsLookupError) ? err.msg : err.to_s
    ...
    sdk_err = DnsLookupError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(DnsLookupError)
