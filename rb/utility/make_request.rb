# DnsLookup SDK utility: make_request
...
module DnsLookupUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DnsLookupResponse.new({})
    ...
    if fetch_err
      ...
      response = DnsLookupResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DnsLookupResponse.new(fetched)
    else
