...
require_relative 'DnsLookup_types'
...
class DnsLookupSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DnsLookupHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DnsLookupHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DnsLookupFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DnsLookupUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DnsLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DnsLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DnsLookupHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DnsLookupHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DnsLookupError.new(
        "graphql_error", "DnsLookupSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
