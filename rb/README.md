...
The Ruby SDK for the DnsLookup API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
require_relative "DnsLookup_sdk"
client = DnsLookupSDK.new(options)
```
...
```ruby
client = DnsLookupSDK.test(testopts, sdkopts)
```
...
raise a `DnsLookupError` (a `StandardError` subclass), so wrap
...
```
rb/
├── DnsLookup_sdk.rb       -- Main SDK module
...
The main module (`DnsLookup_sdk`) exports the SDK class
