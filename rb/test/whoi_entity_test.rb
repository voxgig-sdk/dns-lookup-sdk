...
class WhoiEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DNSLOOKUP_TEST_WHOI_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def whoi_basic_setup(extra)
  ...
  entid_env_raw = ENV["DNSLOOKUP_TEST_WHOI_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "DNSLOOKUP_TEST_WHOI_ENTID" => idmap,
