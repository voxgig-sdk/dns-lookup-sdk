...
class DomainEntityTest < Minitest::Test
  def test_create_instance
    testsdk = DnsLookupSDK.test(nil, nil)
  ...
  def test_stream
    ...
    base = DnsLookupSDK.test(seed, nil)
    seen = base.Domain(nil).stream("list", nil, nil).to_a
    assert_equal 3, seen.length

    # Inbound: streaming active -> yields each item from the feature.
    cfg = DnsLookupConfig.shared_config
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      sdk = DnsLookupSDK.test(seed, { "feature" => { "streaming" => { "active" => true } } })
      got = []
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DNS_LOOKUP_TEST_DOMAIN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def domain_basic_setup(extra)
  ...
  entid_env_raw = ENV["DNS_LOOKUP_TEST_DOMAIN_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["DNS_LOOKUP_TEST_DOMAIN_ENTID"])
