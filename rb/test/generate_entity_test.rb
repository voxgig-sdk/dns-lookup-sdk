...
class GenerateEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DNSLOOKUP_TEST_GENERATE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def generate_basic_setup(extra)
  ...
  entid_env_raw = ENV["DNSLOOKUP_TEST_GENERATE_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["DNSLOOKUP_TEST_GENERATE_ENTID"])
