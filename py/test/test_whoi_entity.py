...
class TestWhoiEntity:
    ...
    def test_should_stream(self):
        ...
        base = DnsLookupSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DNS_LOOKUP_TEST_WHOI_ENTID JSON to run live")
        client = setup["client"]
...
def _whoi_basic_setup(extra):
    ...
    client = DnsLookupSDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("DNS_LOOKUP_TEST_WHOI_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("DNS_LOOKUP_TEST_LIVE") == "TRUE":
        ...
        client = DnsLookupSDK(helpers.to_map(merged_opts))

    _live = env.get("DNS_LOOKUP_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("DNS_LOOKUP_TEST_EXPLAIN") == "TRUE",
        "live": _live,
