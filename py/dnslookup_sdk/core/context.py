...
from dnslookup_sdk.core.helpers import get_ctx_prop, to_map
...
class DnsLookupContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, DnsLookupSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, DnsLookupResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, DnsLookupResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return DnsLookupError(code, msg, self)
