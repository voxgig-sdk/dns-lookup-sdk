...
[MCP](https://modelcontextprotocol.io) server exposing the DnsLookup SDK as
two agent tools — `dns-lookup_list` and `dns-lookup_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dns-lookup-mcp)
make build

# 2. Provide credentials via the environment
export DNS_LOOKUP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dns-lookup \
  -- /absolute/path/to/dns-lookup-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dns-lookup-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dns-lookup_list: first page of records
{ "entity": "domain" }
{ "entity": "domain", "query": { } }

// dns-lookup_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dns-lookup-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dns-lookup \
     -- "$PWD"/dist/*/dns-lookup-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dns-lookup_list` and `dns-lookup_load` tools now appear
   in new sessions. Ask the agent to *"list domain using dns-lookup"*
   and it calls `dns-lookup_list` with `{"entity":"domain"}`.
...
```sh
export DNS_LOOKUP_APIKEY=sk_live_xxx            # API key
export DNS_LOOKUP_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dns-lookup_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dns-lookup_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DNS_LOOKUP_APIKEY` | API key sent with every request. |
| `DNS_LOOKUP_BASE` | Optional override of the API base URL. |
...
```sh
./dns-lookup-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dns-lookup_load","arguments":{"entity":"dns_result","query":{"id":1}}}}'
```
