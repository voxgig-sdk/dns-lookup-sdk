# DnsLookup TypeScript SDK



The TypeScript SDK for the DnsLookup API — a type-safe, entity-oriented client with full async/await support.
...
- Releases: [https://github.com/voxgig-sdk/dns-lookup-sdk/releases](https://github.com/voxgig-sdk/dns-lookup-sdk/releases)
...
```ts
const client = DnsLookupSDK.test()
...
```ts
const client = new DnsLookupSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `DnsLookupSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `DnsLookupSDK.test(testopts?, sdkopts?)` | `DnsLookupSDK` | Create a test-mode client. |
...
| `client` | `client(): DnsLookupSDK` | Return the parent SDK client. |
...
dns-lookup/
├── src/
│   ├── DnsLookupSDK.ts        # Main SDK class
