...
function isRunnable(code) {
    return (/new\s+DnsLookupSDK\b/.test(code) ||
        /\bDnsLookupSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+DnsLookupSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/DnsLookupSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
