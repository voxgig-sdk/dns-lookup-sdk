...
(0, node_test_1.describe)('DomainEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set DNS_LOOKUP_TEST_DOMAIN_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    const idmapEnvVal = process.env['DNS_LOOKUP_TEST_DOMAIN_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'DNS_LOOKUP_TEST_DOMAIN_ENTID': idmap,
        'DNS_LOOKUP_TEST_LIVE': 'FALSE',
        'DNS_LOOKUP_TEST_EXPLAIN': 'FALSE',
        'DNS_LOOKUP_APIKEY': '',
    });
    idmap = env['DNS_LOOKUP_TEST_DOMAIN_ENTID'];
