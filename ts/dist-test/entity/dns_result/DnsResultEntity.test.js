...
(0, node_test_1.describe)('DnsResultEntity', async () => {
    ...
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('DNS_LOOKUP_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.DnsLookupSDK.test();
        const ent = testsdk.DnsResult();
        (0, node_assert_1.default)(null != ent);
    });
    (0, node_test_1.test)('basic', async (t) => {
        const live = 'TRUE' === process.env.DNS_LOOKUP_TEST_LIVE;
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set DNS_LOOKUP_TEST_DNS_RESULT_ENTID JSON to run live');
            return;
        }
...
function basicSetup(extra) {
    ...
    let client = __1.DnsLookupSDK.test(options, extra);
    ...
    const idmapEnvVal = process.env['DNS_LOOKUP_TEST_DNS_RESULT_ENTID'];
    const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{');
    const env = (0, utility_1.envOverride)({
        'DNS_LOOKUP_TEST_DNS_RESULT_ENTID': idmap,
        'DNS_LOOKUP_TEST_LIVE': 'FALSE',
        'DNS_LOOKUP_TEST_EXPLAIN': 'FALSE',
        'DNS_LOOKUP_APIKEY': '',
    });
    idmap = env['DNS_LOOKUP_TEST_DNS_RESULT_ENTID'];
    const live = 'TRUE' === env.DNS_LOOKUP_TEST_LIVE;
    if (live) {
        client = new __1.DnsLookupSDK(merge([
    ...
    const setup = {
        ...
        struct,
        data: entityData,
        explain: 'TRUE' === env.DNS_LOOKUP_TEST_EXPLAIN,
        live,
